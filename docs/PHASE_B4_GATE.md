...
> **Wave 5+**: Trade Celery/Flower retired; `/api/ops/` retains auth, audit, market-ingest only.
...
2. SSE: 19 streams in one process — accepted; same GIL model as today across pods;
   Ops log SSE uses unbounded proxy timeout.
3. Blast radius: operations outage = monitor + ops control together — accepted for
   aggressive Phase B (Owner chose 4-domain target).
4. Smoke after deploy: /api/monitor/health, /api/ops/health, log SSE.
